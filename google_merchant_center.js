...
function syncGMCWithUI() {
    ...
    if (result.status === 'SUCCESS') {
        ...
        const message = `✅ Merchant Center Synchronized Successfully\n\n` +
            `🛍️ Accounts: ${details.accounts}\n` +
            `📊 Data Sources: ${details.dataSources}\n` +
            `📦 Products identified: ${details.products}\n\n` +
            `Total: ${result.records} elements\n` +
            `Time: ${Math.round(result.duration / 1000)}s`;
        ui.alert('🛍️ Merchant Center Completed', message, ui.ButtonSet.OK);
    } else {
        ui.alert(
            '❌ Merchant Center Error',
