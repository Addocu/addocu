...
function syncBigQueryWithUI() {
    ...
    if (result.status === 'SUCCESS') {
        ...
        const message = `✅ BigQuery Synchronized Successfully\n\n` +
            `🗄️ Datasets: ${details.datasets}\n` +
            `📊 GA4 Tables: ${details.ga4Tables}\n` +
            `🔗 Export Links: ${details.exportLinks}\n\n` +
            `Total: ${result.records} elements\n` +
            `Time: ${Math.round(result.duration / 1000)}s`;
        ui.alert('🗄️ BigQuery Completed', message, ui.ButtonSet.OK);
    } else if (result.status === 'SKIPPED') {
        ui.alert(
            '⚠️ BigQuery Skipped',
            'No BigQuery Project ID configured.\n\nGo to "Configure Addocu" > Advanced Filters to set your GCP Project ID.',
            ui.ButtonSet.OK
        );
    } else {
        ui.alert(
