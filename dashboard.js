...
function getGSCStats() {
  try {
    ...
    sheets.forEach(sheetName => {
      const count = countRecords(sheetName);
      details[sheetName.replace('GSC_', '').toLowerCase()] = count;
      totalElements += count;
    });

    return {
      totalElements: totalElements,
      sites: details.sites || 0,
      sitemaps: details.sitemaps || 0,
      status: totalElements > 0 ? '✅ Active' : '⚠️ No Data',
      lastUpdate: getLastUpdate('GSC'),
      qualityScore: 'N/A'
    };

  } catch (error) {
    logError('DASHBOARD_V3', `Error getting Search Console stats: ${error.message}`);
    return { totalElements: 0, status: '❌ Error', lastUpdate: 'Error' };
  }
}
...
/**
 * Collects statistics for YouTube.
 * @returns {Object} YouTube statistics.
 */
function getYouTubeStats() {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const channelsSheet = ss.getSheetByName('YOUTUBE_CHANNELS');
    const playlistsSheet = ss.getSheetByName('YOUTUBE_PLAYLISTS');

    const totalChannels = channelsSheet ? Math.max(0, channelsSheet.getLastRow() - 1) : 0;
    const totalPlaylists = playlistsSheet ? Math.max(0, playlistsSheet.getLastRow() - 1) : 0;

    return {
      channels: totalChannels,
      playlists: totalPlaylists,
      totalElements: totalChannels + totalPlaylists,
      status: totalChannels > 0 ? '✅ Active' : '❌ Not Connected',
      lastUpdate: getLastUpdate('youtube'),
      qualityScore: "100%"
    };
  } catch (e) {
...
/**
 * Collects statistics for Google Business Profile.
 * @returns {Object} GBP statistics.
 */
function getGBPStats() {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    const accountsSheet = ss.getSheetByName('GBP_ACCOUNTS');
    const locationsSheet = ss.getSheetByName('GBP_LOCATIONS');

    const totalAccounts = accountsSheet ? Math.max(0, accountsSheet.getLastRow() - 1) : 0;
    const totalLocations = locationsSheet ? Math.max(0, locationsSheet.getLastRow() - 1) : 0;

    return {
      accounts: totalAccounts,
      locations: totalLocations,
      totalElements: totalAccounts + totalLocations,
      status: totalAccounts > 0 ? '✅ Active' : '❌ Not Connected',
      lastUpdate: getLastUpdate('googleBusinessProfile'),
      qualityScore: "100%"
    };
  } catch (e) {
